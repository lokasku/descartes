...
let locate (rng : range) (lines : string list) : loc =
...
let body (report : Report.t) (charset : charset) =
  ...
  let sorted_labels = Label.sort report.labels in
  ...
  in
  eprintf "%s\n" file_info ;
  let rec apply (f : 'a -> 'b) (i : int) (lst : 'b list) =
    f @@ List.nth lst i
  in
  (* Print labels *)
  apply
    (fun label ->
      let print_label (multilines : Label.t list) =
        let prefix n =
          " "
          ^ String.make
              (last_number_len - (String.length @@ string_of_int n))
              ' '
          ^ string_of_int (n + 1)
          ^ " " ^ charset.vbar
        in
        match label.loc.start_line = label.loc.end_line with
        | true (* one line label *) ->
            assert false
        | false (* multiline label *) ->
            assert false
      in
      print_label [] )
    0 sorted_labels
